...
c        if co2/ma2
      ...
      include 'rfn_msplit.f'
      include 'my_errchk.f' ! mesh chk
      include 'my_post.f'   ! selected outpost
c-----------------------------------------------------------------------
      subroutine rfn_msplit(Ncut) ! multiple (volumetric) split
      implicit none
      include 'SIZE'
      include 'PARALLEL' ! nelgv
      include 'REFINE'

      integer Nref,ie_ref(lelt),Ncut,Nseg,Nnew_p_e
      real Rref(lref-1),Rlim(4,lref-1)

      Nseg = Ncut + 1 ! # segment per direction
      Nnew_p_e = Nseg*Nseg*Nseg-1 ! each element will create m^3-1 new elements
   
      if (ldim.ne.3) call exitti('Err: msplit only works in 3D$',1)
      if (Ncut.eq.0) return

      call rfn_init

      ! Not supports periodic BC for now FIXME
      call chk_periodic_not_support
...
c-----------------------------------------------------------------------
      ...
      call rfn_chk_Ncut(Ncut,3)         ! also chk size (lelg,lelv)
      call rfn_set_nel_lv_shift         ! prepare new element id offset
...
c-----------------------------------------------------------------------
      ...
      integer ifld,e,f,iglmax,iglsum,ivlsum,ntmp
...
c-----------------------------------------------------------------------
      subroutine chk_periodic_not_support
      implicit none
      include 'SIZE'
      include 'TOTAL'
      integer ierr,e,f,iglsum
...
c     Fill/backup global variables
     ...
     $                          ,nelgv0,', E_lyr=',nelg_lv
      elseif (imode.eq.3) then 
        if (nio.eq.0) write(*,22)'rfn SSPLIT START,   E='
     $                          ,nelgv0,', E_lyr=',nelg_lv,', Ncut',Nref
      elseif (imode.eq.4) then 
        if (nio.eq.0) write(*,22)'rfn MSPLIT START,   E='
     $                          ,nelgv0,', E_lyr=',nelg_lv,', Ncut',Nref
      endif
...
c-----------------------------------------------------------------------
      subroutine rfn_chk_Ncut(Ncut,imode)
      implicit none
      include 'SIZE'
      include 'PARALLEL' ! nelgv
      include 'REFINE'
      integer Ncut,nelv_est,nelgv_est,imode
      ...
      elseif (imode.eq.4) then
        nelv_est = nelv + nel_lv*( (Ncut+1)*(Ncut+1)*(Ncut+1) - 1 )
        nelgv_est = nelgv + nelg_lv*( (Ncut+1)*(Ncut+1)*(Ncut+1) - 1 )
      endif
