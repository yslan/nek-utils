c-----------------------------------------------------------------------
      ...
      subroutine my_gfldr_mesh(meshfld, sourcefld)
c
...
c
      ...
      if(nio.eq.0) then
        write(6,*) 'call gfldr msh',trim(meshfld)
        write(6,*) '           fld',trim(sourcefld)
      endif

      ! open mesh field file
      ierr = 0
      if(nid.eq.0) then
        open (90,file=meshfld,status='old',err=100)
        close(90)
        goto 101
 100    ierr = 1
 101  endif
      call err_chk(ierr,' Cannot open source fld file!$')
      call byte_open_mpi(meshfld,fldh_gfldr,.true.,ierr)
     ...
     $   'ABORT: buffer too small, increase lelt > ', lelt_req
      ...
      if(ifgetxr) then
        ...
        call exitti('meshfld does not contain a mesh!$',0)
      endif
      call byte_close_mpi(fldh_gfldr,ierr)

      ! backup numbers for checks
      nelgs_msh = nelgs
      nels_msh = nels
      nxs_msh = nxs
      nxyzs_msh = nxyzs
 ...
 201  endif
     ...
     $   'ABORT: buffer too small, increase lelt > ', lelt_req
      ...
      ! do more checks
      if (nelgs_msh.ne.nelgs)
     $ call exitti('sourcefld vs meshfld: nelgs mismatched!$',0)
      if (nels_msh.ne.nels)
     $ call exitti('sourcefld vs meshfld: nels mismatched!$',0)
      if (nxs_msh.ne.nxs)
     $ call exitti('sourcefld vs meshfld: nxs mismatched!$',0)
      if (nxyzs_msh.ne.nxyzs)
     $ call exitti('sourcefld vs meshfld: nxyzs mismatched!$',0)

      ifldpos = 0
      if(ifgetxr) then
        ! read source mesh coordinates if present
        call gfldr_getxyz(xm1s,ym1s,zm1s)
        ifldpos = ldim
      endif
